            ...
            <div className=' border border-primary '>
                <div className='lg:px-5 px-2 lg:py-5 py-3'>
                    <div className='text-secondary text-lg font-semibold flex justify-between'>
                        <h2>{singleaddress.name}{" "}{singleaddress?.lastName && singleaddress?.lastName}</h2>
                        {userData && userData?.defaultAddress?.address === singleaddress?.address && <div className=' lg:text-sm text-xs lg:px-5 px-3 py-1.5 rounded-2xl font-medium bg-secondary text-white flex justify-center items-center'><h2>DEFAULT</h2></div>}
                    </div>
                    <div className='my-2 flex flex-col gap-1'>
                        <div className='text-gray-500 text-sm font-semibold '>{singleaddress.address}</div>
                        <div className='text-gray-500 text-sm font-semibold  '>{singleaddress.city}{" "}-{" "}{singleaddress.pincode}</div>
                    </div>
                    <div className='text-secondary text-sm font-semibold'><span className='text-gray-500 font-semibold '>Mobile</span>{" "}:{" "}{singleaddress.phoneNo}</div>
                ...
                <div className='flex w-full bg-[#FFEEF4]  border-t border-t-primary  md:text-base text-sm font-semibold'>
                    <div
                        ...
                        className='w-[50%] flex justify-center text-primary border-r border-r-primary py-3 cursor-pointer'><button>Remove</button></div>
                    <div onClick={() => {
                        handleOpenEditModal()
