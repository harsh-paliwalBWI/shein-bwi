...
import { getUserData, getUserWishlist, removeFromWishListHandler, moveToWishListHandler } from "../../utils/databaseService";
...
const ProductCard = ({ product, idx = Math.random(), mx }: any) => {
  ...
  return (
    ...
    >
      <div
        className={`absolute z-30 right-[25px] top-[25px] ${hoveredProduct === product?.id ? "visible" : "invisible"
      ...
      <div className={`white-triangle flex justify-center items-center border ${hoveredProduct === product?.id ? "border-primary" : "border-secondary"} `}>
        <div
          className={` green-triangle  border  ${hoveredProduct === product?.id
              ? "border-secondary"
              : "border-primary"
